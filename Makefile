...
VERSION := $(shell echo $(shell git describe --always --match "*v*" --tags) | sed 's/^v//')
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=lumenx \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=lumenxd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/lumenxd ./cmd/lumenxd
endif
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/lumenxd
...
draw-deps:
	...
	@goviz -i ./client/lumenxd -d 2 | dot -Tpng -o dependency-graph.png
